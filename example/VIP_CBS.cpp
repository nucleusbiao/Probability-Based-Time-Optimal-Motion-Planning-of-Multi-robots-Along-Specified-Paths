...
struct HighLevelNode {
    ...
    friend std::ostream& operator<<(std::ostream& os, const HighLevelNode& c) {
        ...
        for (size_t i = 0; i < c.robots.size(); ++i) {
            os << c.robots[i]<< std::endl;
...
double caculateCost2( std::vector<std::vector<double>> &seq,
                      double timeStep){
    double cost=0;
    for(size_t i=0;i<=seq.size()-1;i++){
        double tempTime=(seq[i].size()-1)*timeStep;
        cost=cost+tempTime;
    }
}
...
bool PCBS(Constraints& constraints,
    ...
    start.robots.clear();
    for(size_t i=0;i<=robots.size()-1;i++){
        start.robots.push_back(ScenarioUser(robots[i]));
    }
    start.cost = caculateCost(robots,timeStep);
    start.id = 0;
    ...
    while (!open.empty()) {
        ...
        std::cout<<"id: "<<P.id<<"cost  "<<P.cost<<std::endl;
        open.pop();
        Conflicts result;
        if(!getFirstConflict(startStates,goals,P.robots,timeStep,result,positionSeq,sigma,p_)){
            robots.clear();
            for(size_t i=0;i<=P.robots.size()-1;i++){
                robots.push_back(ScenarioUser(P.robots[i]));
            }
        ...
        HighLevelNode newNode1 = P;
        newNode1.id = id;
        newNode1.robots[result.agent2].scimp_controller.addPVTObstacleSet(tl,tu,pl,pu);
        double tempTime=getTimeByPosition(newNode1.robots[result.agent2].constraints.getXLimit(),timeStep,newNode1.robots[result.agent2]);
        //newNode1.cost=std::max(newNode1.cost,tempTime);
        newNode1.cost=std::max(newNode1.cost,tempTime);//-(positionSeq[result.agent2].size()-1)*timeStep;

        if(tempTime>=0){
            auto handle = open.push(newNode1);
            (*handle).handle = handle;
        }

        //constraint 2
        id++;
        for(double time=result.time2;time<positionSeq[result.agent2].size()*timeStep;time+=timeStep){
            double prob=calculateConflictProb(time,timeStep,sigma,positionSeq[result.agent2],result.position2,threshold2);
            if(prob<p_){
                tu=time;
                break;
            }
        }
        for(double time=result.time2;time>0;time-=timeStep){
            double prob=calculateConflictProb(time,timeStep,sigma,positionSeq[result.agent2],result.position2,threshold2);
            if(prob<p_){
                tl=time;
                break;
        ...
        minPj=p_/result.prob2;
        for(double pos=result.position1;pos<positionSeq[result.agent1].back();pos+=pathStep){
            double prob=calculateConflictProb(result.time2,sigma,pos,result.position1,threshold1);
            if(prob<minPj){
                pu=pos;
                break;
            }
        }
        for(double pos=result.position1;pos>0;pos-=pathStep){
            double prob=calculateConflictProb(result.time2,sigma,pos,result.position1,threshold1);
            if(prob<minPj){
                pl=pos;
                break;
        ...
        pl=pl-z;
        pu=pu+z;
        HighLevelNode newNode2 = P;
        newNode2.id = id;
        newNode2.robots[result.agent1].scimp_controller.addPVTObstacleSet(tl,tu,pl,pu);
        tempTime=getTimeByPosition(newNode2.robots[result.agent1].constraints.getXLimit(),timeStep,newNode2.robots[result.agent1]);

        //newNode2.cost=std::max(newNode2.cost,tempTime);
        newNode2.cost=std::max(newNode2.cost,tempTime);
        id++;
        if(tempTime>=0){
            handle = open.push(newNode2);
            (*handle).handle = handle;
        }
...
int main () {
    ...
    try {
        ...
        for(size_t i=0;i<rowNum+colNum;i++){
            ...
            // robots[i].generatePositionSeq(timeStep);
        }
